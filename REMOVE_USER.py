...
def main_func():
    ...
    # 1.host (the console url)
    # 2. KEY (SEC Key / token)
    # 3. refset (the reference set name to remove the user from)

    host = sys.argv[1]
    KEY = sys.argv[2]
    domainUserName = sys.argv[3]
    domainPassword = sys.argv[4]
    refset = sys.argv[5]
    ...
    ## Remove the username resolved from the reference set

    remove_user = 'https://' + host + '/api/reference_data/sets/' + refset + '/' + x
    add_head = {
        'SEC': KEY
    ...
    ## Send the post request, use the  username you found and remove it from the reference set:

    url = remove_user + urllib.parse.urlencode(params)
    new_req = requests.delete(url, headers=add_head, verify=False).json()
